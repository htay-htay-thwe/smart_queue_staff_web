...
import { Bell, CheckCheck, Clock, User, Armchair, AlertCircle, Trash2 } from "lucide-react";
import {
  Sheet,
  SheetContent,
  SheetDescription,
...
import { useNotiStore, type NotificationType, type Notification } from "@/store/notiStore";
...
const getIcon = (type: NotificationType) => {
  switch (type) {
    case "queue":    return <User className="w-5 h-5" />;
    case "seat":     return <Armchair className="w-5 h-5" />;
    case "complete": return <CheckCheck className="w-5 h-5" />;
...
const getIconBg = (type: NotificationType) => {
  switch (type) {
    ...
    case "seat":     return "bg-green-100 text-green-600";
    case "complete": return "bg-purple-100 text-purple-600";
...
export function NotificationBell() {
  // Read directly from store — always up-to-date, no stale local copy
  const notifications  = useNotiStore((s) => s.notifications);
  const markAsRead     = useNotiStore((s) => s.markAsRead);
  const markAllAsRead  = useNotiStore((s) => s.markAllAsRead);
  const clearAll       = useNotiStore((s) => s.clearAll);
              ...
              notifications.map((notification) => (
                ...
                >
                  <div className="flex gap-3">
                    {/* Icon */}
                    <div className={`w-10 h-10 rounded-lg flex items-center justify-center shrink-0 ${getIconBg(notification.type)}`}>
